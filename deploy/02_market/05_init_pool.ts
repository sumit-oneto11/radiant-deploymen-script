...
import { LendingPoolAddressesProvider } from "../../typechain-types";
...
const func: DeployFunction = async function ({
  ...
  const addressesProviderInstance = (
    await getContract("LendingPoolAddressesProvider", addressesProvider)
  ).connect(await hre.ethers.getSigner(deployer)) as LendingPoolAddressesProvider;

  const isPoolProxyPending =
    (await addressesProviderInstance.getLendingPool()) === ZERO_ADDRESS;
  ...
  if (isPoolProxyPending) {
    const setPoolImplTx = await waitForTx(
      await addressesProviderInstance.setLendingPoolImpl(poolImplDeployment.address)
    );
    const txPoolProxyAddress = await addressesProviderInstance.getLendingPool();
    deployments.log(
  ...
  const poolProxyAddress = await addressesProviderInstance.getLendingPool();
  ...
  const isPoolConfiguratorProxyPending =
    (await addressesProviderInstance.getLendingPoolConfigurator()) === ZERO_ADDRESS;
  ...
  if (isPoolConfiguratorProxyPending) {
    const setPoolConfiguratorTx = await waitForTx(
      await addressesProviderInstance.setLendingPoolConfiguratorImpl(
  ...
  const poolConfiguratorProxyAddress =
    await addressesProviderInstance.getLendingPoolConfigurator();
